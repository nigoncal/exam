...
//  r.x <= x <= r.x + r.w||r.y <= y <= r.y + r.h == si ok, pt ds rectangle

//Donc on n'est pas ds le rect si :
...
// x = coordo x case courante / r.x = coordo x cote gauche forme / r.x + r.w = coordo x cote droit forme
// y = coordo y case courante / r.y = coordo y haut forme / r.y + r.h = coordo y bas forme
// si - de 1 de diff entre x et r.x || 
