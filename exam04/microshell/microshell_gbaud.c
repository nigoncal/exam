...
/*   Updated: 2022/05/16 14:23:08 by pmillet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/********		DELIMIT 1		********/
// breaking input into segments, delimited by "|" and ";"
// argv[] = current arg ?
//
...
int main(int ac, char **av, char **env)
{
	...
	// i = on ac (total nb of args, beginning at 1 to skip executable name),
	// j = ? inside argv[x], to look for semi-colons or pipes,
	// k = ,
	// l =
	...
	{
		j = i, k = i, l = i;
		// checks in current av[], if there is a ";" alone at beginning
		while (j < ac && strncmp(av[j], ";", 2))
			j++; // j counts how many significant ";" there are in av[j]
		/*TEST*/ printf("j = %d in %d ac\n", j, i);
		fd_in = 0;
		while (k < j) // Decompose chaque argv[] : avance tant qu'on est pas arrivé au ";" / "|"
		{
			...
			char *argv[l - k + 1]; // creates an array of ... long

			sub(argv, av, k, l);	  // gives our new array argv[], input argument array av[],
